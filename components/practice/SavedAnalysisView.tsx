        ...
        <div className="max-w-7xl mx-auto">
            ...
            {analyses.length === 0 ? (
                <div className="text-center py-20 bg-gray-800/50 border border-gray-700 rounded-lg">
                    <BookmarkSquareIcon className="w-16 h-16 mx-auto text-gray-500 mb-4" />
                    <h2 className="text-2xl font-bold text-white">No Saved Analyses Yet</h2>
                    <p className="text-gray-400 mt-2">Go to the Trade Simulator, complete a scenario, and click "Save Analysis" to see it here.</p>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                    {analyses.map(item => (
                        <div key={item.id} className="bg-gray-800/70 border border-gray-700 rounded-lg overflow-hidden shadow-lg animate-[fade-in_0.5s_ease-out]">
