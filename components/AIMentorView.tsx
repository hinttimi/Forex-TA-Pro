...
const EXAMPLE_PROMPTS = [
    ...
    {
        icon: BeakerIcon,
        text: "Take me to the AI Strategy Lab to backtest an idea."
    },
    {
        icon: RocketLaunchIcon,
        text: "Let's practice in the Trade Simulator."
    },
    {
        icon: MagnifyingGlassIcon,
        text: "Why is EUR/USD moving right now?"
    },
]
...
export const AIMentorView: React.FC<AIMentorViewProps> = ({ onSetView, onExecuteTool }) => {
    ...
    const getVoiceButton = () => {
        const classMap = {
            idle: 'bg-slate-700 text-slate-300 hover:bg-cyan-500 hover:text-slate-900',
    ...
    const WelcomeScreen = () => (
        <div className="text-center m-auto">
            ...
            <p className="mt-2 text-lg text-slate-400">Ask me anything, or try one of the examples below.</p>
            <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-4 max-w-2xl mx-auto">
                {EXAMPLE_PROMPTS.map((prompt, index) => (
                    <button
                        key={index}
                        onClick={() => setUserInput(prompt.text)}
                        className="p-4 bg-slate-800 border border-slate-700 rounded-lg text-left shadow-sm hover:border-cyan-500/50 hover:-translate-y-0.5 transition-all"
                    >
                ...
                {messages.map((msg) => (
                    <div key={msg.id} className={`flex items-start gap-4 animate-[fade-in_0.5s_ease-out] ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                       ...
                       <div className={`w-full max-w-lg p-4 rounded-xl shadow-sm ${msg.role === 'user' ? 'bg-slate-700' : 'bg-slate-800'}`}>
                           ...
                           {msg.file && !msg.file.mimeType.startsWith('image/') && (
                                <div className="mb-3 p-3 bg-slate-800/60 border border-slate-600 rounded-lg flex items-center gap-3">
                                    <DocumentTextIcon className="w-8 h-8 text-slate-400 flex-shrink-0" />
                                    <span className="text-sm text-slate-300 truncate font-medium">{msg.file.name}</span>
                                </div>
                           )}
                           {msg.isImageLoading && (
                               <ChartDisplay
                                   ...
                                   containerClassName="mb-3 rounded-lg overflow-hidden border border-slate-600"
                               />
                           )}
                           {msg.modelResponseImage && !msg.isImageLoading && (
                               <ChartDisplay
                                   ...
                                   isLoading={false}
                                   containerClassName="mb-3 rounded-lg overflow-hidden border border-slate-600 hover:border-cyan-500/50 transition-colors"
                               />
                           )}
                           {msg.text && <div className="prose prose-invert prose-sm max-w-none text-slate-200"><FormattedContent text={msg.text} /></div>}
                           {msg.groundingChunks && msg.groundingChunks.length > 0 && (
                                <div className="mt-4 pt-3 border-t border-slate-700">
                                    <h4 className="text-xs font-semibold text-slate-400 uppercase mb-2 flex items-center"><LinkIcon className="w-4 h-4 mr-1.5" /> Sources</h4>
                 ...
                 {isLoading && (
                    <div className="flex items-start gap-4 justify-start">
                        ...
                        <div className="w-full max-w-lg p-4 rounded-xl bg-slate-800 flex items-center space-x-2"><LoadingSpinner /><span className="text-slate-400 text-sm">AI Mentor is thinking...</span></div>
                    </div>
                )}
                 {error && <div className="text-center text-red-400 bg-red-500/10 p-3 rounded-lg">{error}</div>}
                 {voiceState !== 'idle' && (
                    <div className="sticky bottom-0 bg-slate-900/80 backdrop-blur-sm p-3 rounded-lg border border-slate-700">
                        <p className="text-sm text-slate-400"><strong>You:</strong> {currentInputTranscription}<span className="inline-block w-1.5 h-4 bg-cyan-400 ml-1 animate-pulse"></span></p>
                ...
                <div className="bg-slate-800 border border-slate-700 rounded-xl p-2 flex flex-col">
                    {uploadedFile && (
                         <div className="p-2 relative w-fit">
                            {uploadedFile.mimeType.startsWith('image/') ? (
                                <img src={`data:${uploadedFile.mimeType};base64,${uploadedFile.data}`} alt="Preview" className="max-h-24 w-auto rounded-md border border-slate-600" />
                            ) : (
                                <div className="flex items-center gap-3 p-2 bg-slate-700 rounded-md">
                            ...
                            <button onClick={() => setUploadedFile(null)} className="absolute -top-1 -right-1 bg-slate-600 rounded-full p-0.5 text-white hover:bg-red-500"><XMarkIcon className="w-4 h-4" /></button>
                        </div>
                    )}
                    <div className="flex items-end space-x-2">
                        ...
                        <button
                            ...
                            className={`p-2.5 rounded-full transition-colors ${isTtsEnabled ? 'bg-cyan-500/20 text-cyan-400' : 'text-slate-400 hover:text-cyan-400'}`}
                        ...
                        {getVoiceButton()}
                        <button onClick={handleSendMessage} disabled={(!userInput.trim() && !uploadedFile) || isLoading || voiceState !== 'idle'} className="p-2.5 rounded-full bg-cyan-500 text-slate-900 disabled:bg-slate-600 disabled:cursor-not-allowed hover:bg-cyan-400 transition-colors" aria-label="Send message"><PaperAirplaneIcon className="w-6 h-6" /></button>
                    </div>
                </div>
            </div>
            <style>{`
