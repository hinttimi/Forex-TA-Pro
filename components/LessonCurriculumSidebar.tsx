...
export const LessonCurriculumSidebar: React.FC<LessonCurriculumSidebarProps> = ({
  ...
  return (
    <div className="w-full bg-slate-800 border border-slate-700/60 rounded-xl p-4 shadow-none">
      <h3 className="text-lg font-bold text-white px-2 mb-4">Course Content</h3>
      <div className="space-y-2 max-h-[70vh] overflow-y-auto">
        {learningPaths.filter(p => p.isFoundation).flatMap(p => p.modules).map(module => {
          ...
          return (
            <div key={module.title}>
              <button
                ...
                className="w-full flex flex-col p-2 text-left rounded-lg hover:bg-slate-700/50"
              ...
              >
                ...
                <div className="text-xs text-slate-500 mt-1">{completedInModule} / {totalInModule} completed</div>
              </button>
              {isExpanded && (
                      ...
                      <li key={lesson.key}>
                        <button
                          ...
                          className={`w-full text-left p-2 rounded-md text-sm transition-colors flex items-start ${
                            isSelected ? 'bg-cyan-500/10 text-cyan-300' : 'text-slate-300 hover:bg-slate-700/70'
                          }`}
                        >
                          <CheckCircleIcon className={`w-4 h-4 mr-2.5 mt-0.5 flex-shrink-0 ${isCompleted ? 'text-[--color-focus-gold]' : 'text-slate-600'}`} />
