...
export const LessonCurriculumSidebar: React.FC<LessonCurriculumSidebarProps> = ({
  ...
  return (
    <div className="w-full bg-slate-800/70 backdrop-blur-lg border border-slate-700/60 rounded-xl p-4">
                      ...
                      <li key={lesson.key}>
                        <button
                          ...
                          className={`w-full text-left p-2 rounded-md text-sm transition-colors flex items-start ${
                            isSelected ? 'bg-cyan-500/10 text-cyan-300' : 'text-slate-300 hover:bg-slate-700'
                          }`}
                        >
                          <CheckCircleIcon className={`w-4 h-4 mr-2.5 mt-0.5 flex-shrink-0 ${isCompleted ? 'text-cyan-400' : 'text-slate-600'}`} />
